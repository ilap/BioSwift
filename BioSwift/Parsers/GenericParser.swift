...
public class GenericParser<T>: ParserProtocol {
    ...
    public func convertToObject(records: [String]) throws -> T? {
        assertionFailure("BIOSWIFT ERROR: this method should not be called directly!")
        return nil
    }

    private func lineToArray(line: String) -> [String]? {
        ...
        let array = line.componentsSeparatedByString(self.separator).filter {
            (element) -> Bool in
            !element.isEmpty
        }
    ...
    public func parse(fileName: String? = nil) {
        guard let _ = fileName, let contents = try? String(contentsOfFile: fileName!,  encoding: NSASCIIStringEncoding) else { return }
